...
class StoredFan:
    ...
    def __init__(self, fan, center_edge, side_edge_0, side_edge_1,
                 center_edge_cost, side_edge_0_cost, side_edge_1_cost):
        self.fan = fan
...
class HalinGraphTspSolver:
    ...
    def shrink_fan(self, fan_center, fan_center_parent):
        ...
        current_fan = self.graph.subgraph([fan_center] + leaves)
        ...
        cycle_edges = [edge for edge in current_fan.edges if edge[0] != fan_center and edge[1] != fan_center]
        cycle_cost = 0
        for edge in cycle_edges:
        ...
        cost_of_path_through_side_edges = self.cost_two_side_edges_path(cycle_cost, cycle_edges, fan_center)
    ...
    def solve_tsp_for_wheel(self):
        cycle_edges = [edge for edge in self.graph.edges if edge[0] != self.center_node and edge[1] != self.center_node]
        min_delta = 999999
        min_delta_edge = -1
        for edge in cycle_edges:
    ...
    def solve_tsp_recursively(self, current_node, current_node_parent=-1):
        ...
        for child in self.tree.neighbors(current_node):
            if child == current_node_parent:
                continue
            leaf = False
            self.solve_tsp_recursively(child, current_node)
